...
public class CreateBeanApp {
	public static void main(String[] args) {
		...
		//Spring Container 생성 후 - Bean Configuration File 읽기 후
		...
		// => Bean Configuration File의 bean 엘리먼트에 등록된 클래스로 미리 객체를 생성하지 않고 getBean() 호출시 생성 
		BeanFactory factory=new XmlBeanFactory
				(new FileSystemResource("src/main/resources/04-1_beanCreate.xml"));
		System.out.println("========== Spring Container 초기화 후 ==========");
		//BeanFactory.getBean(String beanName) : beanName를 전달하여 Spring Container로부터
		//해당 Spring Bean를 검색하여 반환하는 메소드
